...
class Quaternions:
    ...
    def euler(self, order='xyz', epsilon=0):
        ...
        if order == 'xyz':
            ...
            es[..., 1] = np.arcsin(np.clip(2 * (q1 * q3 + q0 * q2), -1 + epsilon, 1 - epsilon))
            es[..., 2] = np.arctan2(2 * (q0 * q3 - q1 * q2), 1 - 2 * (q2 * q2 + q3 * q3))
        elif order == 'yzx':
            es[..., 0] = np.arctan2(2 * (q0 * q1 - q2 * q3), 1 - 2 * (q1 * q1 + q3 * q3))
            es[..., 1] = np.arctan2(2 * (q0 * q2 - q1 * q3), 1 - 2 * (q2 * q2 + q3 * q3))
            es[..., 2] = np.arcsin(np.clip(2 * (q1 * q2 + q0 * q3), -1 + epsilon, 1 - epsilon))
        elif order == 'zxy':
            es[..., 0] = np.arcsin(np.clip(2 * (q0 * q1 + q2 * q3), -1 + epsilon, 1 - epsilon))
            es[..., 1] = np.arctan2(2 * (q0 * q2 - q1 * q3), 1 - 2 * (q1 * q1 + q2 * q2))
            es[..., 2] = np.arctan2(2 * (q0 * q3 - q1 * q2), 1 - 2 * (q1 * q1 + q3 * q3))
        elif order == 'xzy':
            es[..., 0] = np.arctan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q3 * q3))
            es[..., 1] = np.arctan2(2 * (q0 * q2 + q1 * q3), 1 - 2 * (q2 * q2 + q3 * q3))
            es[..., 2] = np.arcsin(np.clip(2 * (q0 * q3 - q1 * q2), -1 + epsilon, 1 - epsilon))
        elif order == 'yxz':
            es[..., 0] = np.arcsin(np.clip(2 * (q0 * q1 - q2 * q3), -1 + epsilon, 1 - epsilon))
            es[..., 1] = np.arctan2(2 * (q1 * q3 + q0 * q2), 1 - 2 * (q1 * q1 + q2 * q2))
            es[..., 2] = np.arctan2(2 * (q1 * q2 + q0 * q3), 1 - 2 * (q1 * q1 + q3 * q3))
        elif order == 'zyx':
            es[..., 0] = np.arctan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2))
            es[..., 1] = np.arcsin(np.clip(2 * (q0 * q2 - q1 * q3), -1 + epsilon, 1 - epsilon))
            es[..., 2] = np.arctan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2 * q2 + q3 * q3))
        else:
            raise Exception('Order not understood. Must be one of xyz, yzx, zxy, xzy, yxz, zyx.')

        # Following did not match with Matlab
        # if order == 'xyz':
        #     es[..., 0] = np.arctan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2))
    ...
    def slerp(cls, q0s, q1s, a):
        ...
        linear = (1.0 - q_len) < 0.01
        omegas = np.arccos(q_len[~linear])
    ...
    @classmethod
    def between(cls, v0s, v1s, rot_axes):
        rot_axes_norms = np.linalg.norm(rot_axes, axis=-1)
        no_rot_idx = np.argwhere(rot_axes_norms < 1e-6)[..., 0]
        rot_axes_unit = rot_axes / rot_axes_norms[..., None]
        v0s_ortho = v0s - np.einsum('...i,...i->...', v0s, rot_axes)[..., None] * rot_axes_unit
        v1s_ortho = v1s - np.einsum('...i,...i->...', v1s, rot_axes)[..., None] * rot_axes_unit
        thetas = np.arccos(np.einsum('...i,...i->...',
                                     v0s_ortho / np.linalg.norm(v0s_ortho, axis=-1)[..., None],
                                     v1s_ortho / np.linalg.norm(v1s_ortho, axis=-1)[..., None]))
        q = Quaternions.from_angle_axis(thetas, rot_axes_unit)
        if len(no_rot_idx) > 0:
            q[no_rot_idx] = Quaternions.id(len(no_rot_idx))
        return q

    # @classmethod
    # def between(cls, source, dest, fallback_axis=None):
    #     d = np.dot(source, dest) / (np.linalg.norm(source) * np.linalg.norm(dest))
    #     if d > 1. - 1e-6:
    #         return Quaternions.id(1)
    #     if d < 1e-6 - 1.:
    #         if fallback_axis is None:
    #             a = np.cross(source, dest)
    #             if np.linalg.norm(a) == 0.:
    #                 a += 1e-6
    #             return Quaternions.from_angle_axis(np.pi, a)
    #         else:
    #             return Quaternions.from_angle_axis(np.pi, fallback_axis)
    #     else:
    #         a = np.cross(source, dest)
    #         w = np.sqrt((source**2).sum(axis=-1) * (dest**2).sum(axis=-1)) + (source * dest).sum(axis=-1)
    #         return Quaternions(np.concatenate([w[..., np.newaxis], a], axis=-1)).normalized()
